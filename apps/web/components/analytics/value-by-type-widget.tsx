...
export function ValueByTypeWidget({ data }: { data: Array<{ contractType: string; totalValue: number; count: number }> }) {
  ...
  const chartData = [...data].sort((a, b) => b.totalValue - a.totalValue).map((item) => ({ ...item, label: item.contractType === "UNCATEGORIZED" ? typeT("OTHER") : KNOWN_TYPES.has(item.contractType) ? typeT(item.contractType as ContractType) : item.contractType }))
