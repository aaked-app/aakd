...
| Latest `main` Compose replay attempts (`a5690fd`) | INCONCLUSIVE (host capacity) | Two fresh disposable projects were attempted. The first exhausted Docker storage during worker-image export; after reclaiming only 2.661 GB of recoverable BuildKit cache, the second completed both app and worker builds but exhausted the 16 GB Docker VM while unpacking the LibreOffice layer into the app image. Both disposable projects were removed; no application assertion failed. A current-head Compose replay remains required on a host with sufficient Docker storage. |
...
- Two latest current-head disposable replays could not complete because the local 16 GB Docker VM exhausted storage during image export/unpack; this is an environment limitation, not a passing replay. The images themselves build successfully before the final unpack step.
