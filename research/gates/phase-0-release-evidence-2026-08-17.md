...
| Unit/integration tests | PASS | 56 test files, 1,124 tests passed on commit `d8cf5b9`, including MCP detail redaction, request attribution, responsive search behavior, individual AI review, duplicate-action guards, and the no-provider cited extraction fallback |
...
| Current-head Compose replay at `d8cf5b9` | PASS (split build, host constrained) | Disposable project with empty volumes built the slim app and worker images separately, applied all 29 migrations, returned HTTP 200 with DB and Redis green, kept the worker running, processed PDF extraction/document conversion/local cited extraction, and passed the full 18-test browser suite. A single `docker compose up --build` still exceeds this machine's 16 GB Docker storage; the split build is the reproducible path on this host. |
