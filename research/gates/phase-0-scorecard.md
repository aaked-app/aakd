...
| Clean install | 1 clean environment reaches the app using documented steps and accepts a real PDF/DOCX | Current-head isolated Compose replay on 2026-08-17 built app and worker images, applied migrations, kept worker healthy, and returned `/api/health` 200 with DB and Redis green from empty volumes | Release engineering core | Fix setup/docs and rerun on a clean host |
...
Engineering release decision: PASS for the committed OSS core at current-head runtime boundary `c904771`. The isolated Compose replay passed from empty volumes, the 18-test browser suite passed against the running stack, and the reproducible verifier also passes the current 55-file/1,080-test source boundary. The HTTP MCP replay remains verified at `d2fccc9`. A separate cloud VM remains useful for deployment diversity but is not required to establish local self-hosting readiness. Product evidence remains OPEN.
...
| 2026-08-17 | 1 | ENGINEERING PASS / PRODUCT OPEN | Release head `c4d17ef` retains the verified production build, typecheck, lint, 52-file/1,068-test suite, isolation, current-head clean-volume Compose health, stable worker runtime, full 18-test browser suite, explicit MCP text-read authorization and minimized agent projections. Public copy remains aligned across all five locales with the Phase 0 capability matrix. Customer failure, corpus, funded-pilot, repeat-use and support-burden evidence remain uncollected. | Founder/CEO |
| 2026-08-17 | 1 | ENGINEERING PASS / PRODUCT OPEN | Runtime boundary `d2fccc9` remains the verified clean Compose/MCP replay; the latest source verifier passes the 55-file/1,080-test suite, typecheck, lint and production build. Synthetic corpus, activation event contract, module classification, public truth and security checklist are documented. Customer failure, corpus, funded-pilot, repeat-use and support-burden evidence remain uncollected. | Founder/CEO |
| 2026-08-17 | 1 | ENGINEERING PASS / PRODUCT OPEN | Current head `c904771` passes clean-volume Compose health, stable worker runtime and the 18-test browser suite; the latest source verifier passes the 55-file/1,080-test suite, typecheck, lint and production build. The HTTP MCP replay remains verified at `d2fccc9`. Synthetic corpus, activation event contract, module classification, public truth and security checklist are documented. Customer failure, corpus, funded-pilot, repeat-use and support-burden evidence remain uncollected. | Founder/CEO |
