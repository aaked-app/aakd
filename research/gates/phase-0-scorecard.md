...
Engineering release decision: PASS for the committed OSS core at `c4d17ef`. The current-head isolated Compose replay passed from empty volumes on the local Docker engine; a separate cloud VM remains useful for deployment diversity but is not required to establish local self-hosting readiness. Product evidence remains OPEN.
