...
Engineering release decision: PASS for the committed OSS core at runtime boundary `d2fccc9`. The isolated Compose replay passed from empty volumes on the local Docker engine; the reproducible verifier also passes the current 54-file/1,076-test source boundary. A separate cloud VM remains useful for deployment diversity but is not required to establish local self-hosting readiness. Product evidence remains OPEN.
...
| 2026-08-17 | 1 | ENGINEERING PASS / PRODUCT OPEN | Runtime boundary `d2fccc9` remains the verified clean Compose/MCP replay; the latest source verifier passes the 54-file/1,076-test suite, typecheck, lint and production build. Synthetic corpus, activation event contract, module classification, public truth and security checklist are documented. Customer failure, corpus, funded-pilot, repeat-use and support-burden evidence remain uncollected. | Founder/CEO |
