...
| Browser boundary | Cookie mutations reject cross-origin origins; authenticated/API responses are non-cacheable; correlation IDs are bounded and log-safe | `lib/security/request.ts`, middleware, request-security tests | PASS |
| Response headers | App and reverse proxy send HSTS (proxy), nosniff, frame, referrer, permissions, and cross-origin isolation headers | `middleware.ts`, `Caddyfile` | PASS |
| Abuse controls | MCP payloads are capped at 1 MB; production rate limiting fails closed when Redis is unavailable | MCP route, `lib/rate-limit.ts`, rate-limit tests | PASS |
...
| Dependency vulnerabilities | Production dependency audit has no critical advisories and no unresolved direct high advisories | `pnpm audit --prod` on 2026-08-17 | FAIL: Next.js 14 direct high advisories remain; transitive high advisories remain |
...
The current release is not security-complete while the dependency row is FAIL.
The required follow-up is a dedicated Next.js 15 migration with route-handler
and test-fixture compatibility work, followed by a fresh dependency audit and
external penetration test before any enterprise security claim.
